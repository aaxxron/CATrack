% This function implements the ADTrack.
...
for frame = 1:num_frames
    ...
    %     if ~mod(frame,20)
    %         frame_R = frame - 19;
    %         im_R = load_image(params,frame_R,colorImage);
    % %        Log_Rate(im,im_R,params);
    %     end
    if frame > 1
        frame_R = frame - 1;
        im_R = load_image(params,frame_R,colorImage);
...
%% main loop
    if frame > 1
        ...
        [xtf,xtf_m,pos,translation_vec] = run_detection(im_R,im,pos,sz,target_sz,currentScaleFactor,features,cos_window,g_f,g_f_m,global_feat_params,use_sz,ky,kx,newton_iterations,featureRatio,Vy,Vx,params.weight);
        Vy=pos(1)-pos_pre(1);
        Vx=pos(2)-pos_pre(2);
    ...
    if frame==1 
            % extract training sample image region
             ...
             %
             [enhanced_pixels,mask] = SSE(pixels,round(target_sz/currentScaleFactor));
