function [xtf,xtf_m,pos,translation_vec] = run_detection(im_R,im,pos,sz,target_sz,currentScaleFactor,features,cos_window,g_f,g_f_m,global_feat_params,use_sz,ky,kx,newton_iterations,featureRatio,Vy,Vx,weight)
        center=pos+[Vy Vx];
        pixel_template=get_pixels(im, center, round(sz*currentScaleFactor), sz);  
        %V1.0
        pixel_template_R=get_pixels(im_R, center, round(sz*currentScaleFactor), sz);  % I
        
        %V1.0
        [i,m] = LLLSSE(round(target_sz/currentScaleFactor),pixel_template,pixel_template_R);
        
        i=im2uint8(i);
        xt=get_features(i,features,global_feat_params);
